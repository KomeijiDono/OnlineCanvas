...
async function selectData(query, params = []) {
    try {
        ...
        console.error("Select data failed: ", err);
        return [];
    }
}
...
async function checkGroupExists(groupId) {
    try {
        const result = await selectData(
            'SELECT COUNT(*) AS count FROM Groups WHERE groupId = @groupId',
            [{ name: 'groupId', type: sql.VarChar, value: groupId }]
        );
        return result[0].count > 0;
    } catch (err) {
        console.error("检查组是否存在失败: ", err);
        return false;
    }
}
...
async function getGroupMembers(groupId) {
    try {
        ...
        console.error("获取组成员失败: ", err);
        return [];
    }
}

// 辅助函数：检查用户是否在组内
async function checkUserInGroup(groupId, username) {
    try {
        const result = await selectData(
            'SELECT COUNT(*) AS count FROM GroupMembers WHERE groupId = @groupId AND username = @username',
            [
                { name: 'groupId', type: sql.VarChar, value: groupId },
        ...
        console.error("检查用户是否在组内失败: ", err);
        return false;
    }
}

// 辅助函数：获取组的绘制数据
async function getGroupDrawings(groupId) {
    try {
        const result = await selectData(
            'SELECT drawingType, drawingData FROM Drawings WHERE groupId = @groupId',
            [{ name: 'groupId', type: sql.VarChar, value: groupId }]
        );
        return result.map(row => ({
            drawingType: row.drawingType,
            drawingData: JSON.parse(row.drawingData)
        }));
    } catch (err) {
        console.error("获取组绘制数据失败: ", err);
        return [];
    }
}

// 辅助函数：添加组成员
async function addGroupMember(groupId, username, isAdmin = false) {
    try {
        return await insertData(
            'INSERT INTO GroupMembers (groupId, username, isAdmin) VALUES (@groupId, @username, @isAdmin)',
            [
                { name: 'groupId', type: sql.VarChar, value: groupId },
                { name: 'username', type: sql.VarChar, value: username },
                { name: 'isAdmin', type: sql.Bit, value: isAdmin }
            ]
        );
    } catch (err) {
        console.error("添加组成员失败: ", err);
        return false;
    }
}
...
async function removeGroupMember(groupId, username) {
    try {
        return await deleteData(
            'DELETE FROM GroupMembers WHERE groupId = @groupId AND username = @username',
            [
                { name: 'groupId', type: sql.VarChar, value: groupId },
                { name: 'username', type: sql.VarChar, value: username }
            ]
...
async function updateGroupAdmin(groupId, newAdmin) {
    try {
        return await updateData(
            'UPDATE Groups SET admin = @newAdmin WHERE groupId = @groupId',
            [
                { name: 'groupId', type: sql.VarChar, value: groupId },
                { name: 'newAdmin', type: sql.VarChar, value: newAdmin }
            ]
        );
    } catch (err) {
        console.error("更新组管理员失败: ", err);
        return false;
    }
}
